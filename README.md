...
5. Для поиска в каталоге включён двуязычный режим:
   - к названиям товаров добавляются скрытые RU/EN-алиасы,
   - русский запрос в поисковом поле автоматически нормализуется в английский (через словарь и API fallback), если это нужно.
6. Словарь можно автоматически дополнять из Google Sheets (CSV) без правки кода.
...
| `ENABLE_BILINGUAL_SEARCH` | Включает двуязычный слой поиска (алиасы + нормализация RU-запросов) |
| `SEARCH_INPUT_SELECTORS` | CSS-селекторы полей поиска |
| `SEARCH_PRODUCT_TITLE_SELECTORS` | CSS-селекторы заголовков карточек товара |
| `GOOGLE_SHEETS_SYNC_ENABLED` | Включает синхронизацию словаря из Google Sheets |
| `GOOGLE_SHEETS_CSV_URL` | Публичная ссылка CSV на лист таблицы |
| `GOOGLE_SHEETS_FETCH_TIMEOUT_MS` | Таймаут загрузки CSV |
| `GOOGLE_SHEETS_SYNC_TTL_MS` | TTL кэша удалённого словаря в localStorage |

### Google Sheets: формат таблицы

1. Создайте лист с заголовками колонок: `ru` и `en`.
2. Заполняйте строки: русский текст в `ru`, английский перевод в `en`.
3. Опубликуйте лист в CSV:
   - Файл → Поделиться → Опубликовать в интернете,
   - выберите лист и формат CSV.
4. Вставьте полученную ссылку в `GOOGLE_SHEETS_CSV_URL` в `src/config.js`.

Пример:

```csv
ru,en
Платье Dress 01,Dress 01
Куртка Urban,Urban Jacket
```

После публикации менеджер может добавлять новые строки в таблицу, а скрипт будет подхватывать их автоматически (с учётом TTL-кэша).
